...
class MainWin(QtWidgets.QMainWindow):
    ...
    def __init__(self, form_arg):
        ...
        self.log_widget.setPlaceholderText("Журнал сообщений...")

        # Добавляем лог в нижнюю часть основного layout
        ...
        # Находим tabWidget внутри layout и вставляем после него
        index = layout.indexOf(self.tabWidget)
        if index >= 0:
            layout.insertWidget(index + 1, self.log_widget)
        else:
            layout.addWidget(self.log_widget)

        # Устанавливаем пропорции — таблицы занимают всё свободное место, лог фиксирован по высоте
        layout.setStretch(index if index >= 0 else 0, 1)
        layout.setStretchFactor(self.log_widget, 0)
        self.log_widget.setFixedHeight(100)  # высоту можно настроить

        self.append_log("Приложение запущено", "INFO")
    ...
    def append_log(self, message, level='INFO'):
        ...
        self.log_widget.appendPlainText(f"[{timestamp}] {level}: {message}")
    ...
    def readNetlist(self):
        """Чтение и обработка выбранного CSV-файла с полной защитой от падений."""
        ...
        try:
            ...
            self.append_log(f'Файл успешно прочитан: {self.netlist}', 'SUCCESS')

        except Exception as e:
            progress.close()
            error_text = f"Ошибка чтения файла: {self.netlist}\n{e}"
            self.append_log(error_text, 'ERROR')
            self.statusbar.showMessage('Ошибка чтения файла: %s' % str(e))
            QtWidgets.QMessageBox.critical(
                self,
                'Ошибка обработки BOM',
                'Не удалось прочитать файл:\n%s\n\nОшибка:\n%s' % (self.netlist, str(e))
            )
            self.setWindowTitle('altidoc')
            self.setEnabledActions(False)
            self.tabWidget.setEnabled(False)
    ...
    def fillTable(self, table, header, content):
        ...
        col_name = header.index('Наименование') if 'Наименование' in header else -1
        for i, row in enumerate(content):
            for j, cell in enumerate(row[:-1]):
                table.setItem(i, j, QtWidgets.QTableWidgetItem(cell))
            if row[-1] != '' and col_name >= 0:
                item = table.item(i, col_name)
                if item:
    ...
    def readTable(self, table):
        ...
        col_name = header.index('Наименование') if 'Наименование' in header else 0
        content = [[''] * (table.columnCount() + 1) for _ in range(table.rowCount())]
        for i in range(table.rowCount()):
            for j in range(table.columnCount()):
                content[i][j] = table.item(i, j).text() if table.item(i, j) else ''
            item = table.item(i, col_name)
            if item:
                font = item.font()
                if font.underline():
    ...
    def readStamp(self, table):
        table1, table2 = table
        content = {}
        content['number']      = table1.item(0,0).text() if table1.item(0,0) else ''
        content['title']       = table1.item(1,0).text() if table1.item(1,0) else ''
        content['first_usage'] = table1.item(2,0).text() if table1.item(2,0) else ''
        content['company']     = table1.item(3,0).text() if table1.item(3,0) else ''
        content['developer']   = table2.item(0,0).text() if table2.item(0,0) else ''
        content['verifier']    = table2.item(1,0).text() if table2.item(1,0) else ''
        content['inspector']   = table2.item(2,0).text() if table2.item(2,0) else ''
        content['approver']    = table2.item(3,0).text() if table2.item(3,0) else ''
    ...
    def tableChanged(self, i):
        table = self.tables[self.lookup[self.tabWidget.tabText(self.tabWidget.currentIndex())]]
        header = self.readTableHeader(table)
        item = table.item(i, header.index('Наименование') if 'Наименование' in header else 0)
        if item:
            font = item.font()
    ...
    def rebuldPos(self):
        ...
        header = self.readTableHeader(table)
        if doc == 'spec':
            qty_index = header.index('Кол.') if 'Кол.' in header else -1
            pos_index = header.index('Поз.') if 'Поз.' in header else -1
            components_only = True
        elif doc == 'bom':
            qty_index = header.index('Всего') if 'Всего' in header else -1
            pos_index = header.index('№ строки') if '№ строки' in header else -1
        ...
        pos_count = 1
        for i in range(table.rowCount()):
            if table.item(i, qty_index) and table.item(i, qty_index).text() == '' and components_only:
    ...
    def slotSave(self):
        doc = self.lookup[self.tabWidget.tabText(self.tabWidget.currentIndex())]
        number = self.readStamp(self.stamps[doc])['number']
        name = number + '.' + self.extention
        ext_map = {'tex': 'Файл LaTeX (*.tex)', 'pdf': 'Файл PDF (*.pdf)', 'xls': 'Файл Microsoft Excel 2003 (*.xls)', 'csv': 'Файл CSV (*.csv)'}
        ext_str = ext_map.get(self.extention, 'Файл LaTeX (*.tex)')
        (path, filt) = QtWidgets.QFileDialog.getSaveFileName(self, 'Сохранить',
            os.path.join(os.getcwd(), name),
            'Файл LaTeX (*.tex);;Файл PDF (*.pdf);;Файл Microsoft Excel 2003 (*.xls);;Файл CSV (*.csv)', ext_str)
        if path:
            os.chdir(os.path.dirname(path))
            ext_choice = {'Файл LaTeX (*.tex)': 'tex', 'Файл PDF (*.pdf)': 'pdf', 'Файл Microsoft Excel 2003 (*.xls)': 'xls', 'Файл CSV (*.csv)': 'csv'}
            self.extention = ext_choice.get(filt, 'tex')
            doc = self.lookup[self.tabWidget.tabText(self.tabWidget.currentIndex())]
